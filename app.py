...
def holiday_week_ending_for_date(all_week_endings: list[date], holiday_dt: date, holiday_name: str = "") -> Optional[date]:
    """
    Map a holiday date to the chart week_ending date using your workflow rules:

    - Fixed-date holidays (New Year's, Valentine's, Independence Day, Halloween, Christmas):
        * If the holiday is Sun/Mon/Tue/Wed -> use the previous weekend (Saturday on/before the holiday).
        * If the holiday is Thu/Fri/Sat     -> use the following weekend (Saturday on/after the holiday).
    - Thanksgiving: always use the following week_ending (the Saturday after Thanksgiving).
    - Weekend/Monday holidays (Easter, Memorial Day, Labor Day): use the weekend that the holiday is part of
      (Saturday on/before the holiday).

    If an exact Saturday isn't present in the DB, we pick the nearest available week_ending in the required direction.
    """
    ...
    # Helpers: previous Saturday (<=) and next Saturday (>=)
    prev_sat = holiday_dt - timedelta(days=(holiday_dt.weekday() - 5) % 7)
    next_sat = holiday_dt + timedelta(days=(5 - holiday_dt.weekday()) % 7)

    def pick_on_or_before(target: date) -> date:
        i = bisect_right(weeks, target) - 1
        return weeks[i] if i >= 0 else weeks[0]

    def pick_on_or_after(target: date) -> date:
        i = bisect_left(weeks, target)
        return weeks[i] if i < len(weeks) else weeks[-1]

    # Thanksgiving always uses the following chart (Saturday after).
    if "Thanksgiving" in holiday_name:
        return pick_on_or_after(next_sat)

    # Easter/Memorial Day/Labor Day: weekend the holiday is part of (Saturday on/before).
    if holiday_name in weekend_or_monday:
        return pick_on_or_before(prev_sat)

    # Fixed-date holidays with weekday-dependent weekend selection.
    if holiday_name in fixed_any_day:
        if holiday_dt.weekday() in {6, 0, 1, 2}:  # Sun/Mon/Tue/Wed
            return pick_on_or_before(prev_sat)
        return pick_on_or_after(next_sat)  # Thu/Fri/Sat

    # Default (conservative): as-of holiday date -> previous chart.
    return pick_on_or_before(prev_sat)
